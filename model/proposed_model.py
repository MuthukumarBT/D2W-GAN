...
import networks_DTCWT as networks
import losses
import utils
import numpy as np
...
class Model(BaseModel):
    def __init__(self, phase, in_channels=3, out_channels=3, nker=64, lr=0.0002) -> None:
        ...
        self.generator = networks.UnetWithAttentionMultiOut(in_channels + 1, out_channels, nker, attention_mode='RCBAM',
                                                            relu=0.2, use_resize=self.use_resize).to(DEVICE)
    ...
    def set_inputs(self, data):
        # Load input and label images for training
    ...
    def multiscale_loss(self, y_pred_list, y_true):
        loss = 0
        to_pil = ToPILImage()  # To convert tensor to PIL image
        to_tensor = ToTensor()  # To convert back to tensor

        for y_pred in y_pred_list:
            # Convert y_true to PIL Image for resizing (handle batch dimension correctly)
            if y_pred.shape[-2:] != y_true.shape[-2:]:
                # Select the first image in the batch for resizing
                y_true_single = y_true[0].cpu()  # Move the tensor to CPU before using ToPILImage
                y_true_pil = to_pil(y_true_single)

                # Resize the image to match the size of y_pred
                transform = Resize(y_pred.shape[-2:])
                resized_y_true = transform(y_true_pil)

                # Convert resized PIL image back to tensor
                resized_y_true = to_tensor(resized_y_true).unsqueeze(0).to(
                    y_true.device)  # Add batch dimension back and move back to the original device
    ...
    def test_on_batch(self):
        # Perform test or validation for each batch
        self.generator.eval()
        with torch.no_grad():
            self.forward()
    ...
    '''
    Save Training/Testing Results and Losses
    '''

    def get_outputs(self):
        # Prepare the output results for saving
        return {
    ...
    def get_losses(self):
        # Return the current losses in a dictionary format
        return {
